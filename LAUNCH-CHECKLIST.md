# Launch checklist for the author

The site is live on GitHub Pages, but a few things need a human before it goes on the real domain.

## Before showing your prof

- [ ] The author line is on about.html ("This site was created by Ash Akbari..."). Expand it if you want more of your own voice there.
- [ ] Read what-a-court-day-looks-like.html closely. It leans on your firsthand observations; fix anything that does not match what you saw.
- [ ] Skim every page once on your phone.

## Images

- [x] Homepage Fraser River photo placed (assets/fraser-river-new-westminster.webp).
- [x] Getting your case there: Law Courts photo placed.
- [x] Court day: circle room placed, honestly captioned as the Calgary Indigenous Court.
- [x] Healing plans: cedar placed.
- [x] BC map with ten dots: drawn as an inline SVG on the homepage and locations page.
- [x] Court day: Inn at the Quay boardwalk photo placed. Every figure on the site is now filled.
- [ ] Licensing before the real domain launch: the Fraser and cedar photos look like watermarked stock comps (Dreamstime), so buy the licenses and swap the clean files in under the same filenames in assets/. The Law Courts photo looks like a news outlet's and the Inn photo like a travel-site upload; confirm permission or shoot your own. Confirm the Calgary photo's source and permission.
- [ ] Do not use photos of smudging or any ceremony, eagle feathers or drums as props, or stock photos of Indigenous people. Land and water only, unless you have permission.

## Before the real domain goes live

- [ ] Buy firstnationscourt.ca and follow DEPLOY.md to attach it (Kinsta or GitHub Pages both work).
- [ ] Swap the ashakbari.github.io URLs in sitemap.xml and robots.txt for https://firstnationscourt.ca (DEPLOY.md has the exact steps).
- [ ] Dial every phone number on resources.html once to confirm it rings the right place.
- [ ] Re-check the court list and the sitting-dates link against provincialcourt.bc.ca the week you launch. The Chilliwack court is a pilot until March 2027, so check it is still running.
- [ ] Check the Legal Aid BC fact sheet link still works (their site was reorganized once already).

## Ongoing

- [ ] Once or twice a year, re-verify FACTS.md against the live sources and update the "Last updated" line in the footer.
