...
- The nine process pages (including gladue.html) carry the .subnav bar under the header, with aria-current on the current page. Gladue appears in every step flow: subnav, the homepage steps, and how-it-works. The footer's single page list names every page, and the top nav carries .mobile-extra items so the phone menu lists every page too.
- Design language: editorial print warmed by the healing circle. Warm paper, ink-forward type, hairline rules (--rule), letterpress ink buttons (fully rounded), circled step numbers, dot and ring accents instead of straight bars, soft radial warmth in the hero, rounded photo corners. The mark is a healing-circle-inspired abstraction: a ring of dots (people), an inner ring, a warm cedar centre. Never a literal medicine wheel or any sacred symbol as decoration. No rounded-card grids with drop shadows, no loud gradient fills.
...
- Naming: the brand is "Indigenous Court Guide BC", an independent guide, never styled as the court's own site. The domain remains firstnationscourt.ca until the author decides otherwise. Copy must make clear the official category is Indigenous sentencing courts, individual courts are named First Nations Court or Indigenous Court by location, and all of them serve First Nations, Métis, and Inuit people. The footer's independence line appears on every page.
